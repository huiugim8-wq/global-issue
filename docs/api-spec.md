...
3. `GET /api/news/continent/{continent}`
4. `GET /api/news/search`
5. `GET /api/news/{article_id}`
6. `GET /api/news/{article_id}/analysis`
7. `POST /api/articles/save`
8. `GET /api/articles/saved`
9. `DELETE /api/articles/saved/{saved_id}`

이 순서면
**홈 → 카테고리/대륙 탐색 → 검색 → 상세 → 저장** 흐름이 자연스럽게 완성된다.
